...
SUBROUTINE BD_GenerateGLL(N1, GLL_nodes, ErrStat, ErrMsg)
   ...
   CALL AllocAry(GLL_nodes,N1,'GLL points array',ErrStat2,ErrMsg2); CALL SetErrStat( ErrStat2, ErrMsg2, &
...
!bjj: this assumes there is only one member
   ...
   DO j = 2,p%nqp
      ...
      p%QPtN(j) =  InputFileData%InpBl%station_eta(indx) + &
               ((InputFileData%InpBl%station_eta(indx+1) - InputFileData%InpBl%station_eta(indx))/p%refine) * &
               (MOD(j-2,p%refine) + 1)
   ENDDO
